    ...
    public  void recordSale(String productId, int quantity){
        ...
        if(product.getQuantity() < quantity){
            throw  new IllegalArgumentException(
                    String.format("Insufficient stock. Available: %d, Requested: %d",product.getQuantity(),quantity)
            );
        }
        //create a sale record
        String date = LocalDateTime.now().toString();
       Sale sale = new Sale(productId, product.getName(), quantity, product.getPrice(), date);
